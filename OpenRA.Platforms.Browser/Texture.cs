...
namespace OpenRA.Platforms.Browser
{
	sealed class Texture : ThreadAffine, ITextureInternal
	{
		...
		void SetData(IntPtr data, int width, int height)
		{
			PrepareTexture();
#if OPENRA_BROWSER
			// WebGL2 requires sized internal formats; GL_BGRA is not valid as internalFormat.
			if (data == IntPtr.Zero)
			{
			...
			var bpp = width * height * 4;
			var tmp = ArrayPool<byte>.Shared.Rent(bpp);
			try
			{
				Marshal.Copy(data, tmp, 0, bpp);
				for (var i = 0; i < bpp; i += 4)
					(tmp[i], tmp[i + 2]) = (tmp[i + 2], tmp[i]);
				unsafe
				{
					fixed (byte* ptr = tmp)
						OpenGL.glTexImage2D(OpenGL.GL_TEXTURE_2D, 0, OpenGL.GL_RGBA8, width, height,
							0, OpenGL.GL_RGBA, OpenGL.GL_UNSIGNED_BYTE, new IntPtr(ptr));
				}
			}
			finally
			{
...
#if OPENRA_BROWSER
			const int glInternalFormat = OpenGL.GL_RGBA8;
#else
			var glInternalFormat = OpenGL.Profile == GLProfile.Embedded ? OpenGL.GL_BGRA : OpenGL.GL_RGBA8;
#endif
